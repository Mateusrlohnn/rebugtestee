...
public class Room implements Attributable, IRoom {
    ...
    public void addUserEvent(UserWalkEvent task, int ticks) {
        synchronized (task) {
            this.userEvents.entrySet().removeIf(entry -> entry.getValue() == task);
            task.Ticks = ticks;
            final int eventId = this.eventIdGeneratorUsers.updateAndGet(value -> (value + 1) % 999999);
            this.userEvents.put(eventId, task);
    ...
    public Room load() {
        ...
        this.process = (this.data.getRoomProcessType() == RoomProcessingType.CLICK ||
                this.data.getRoomProcessType() == RoomProcessingType.PRESSURE)
        ...
        this.eventIdGeneratorUsers = new AtomicInteger(0);
